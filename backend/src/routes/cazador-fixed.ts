...
// ═══════════════════════════════════════════════════════════════════════════════
// TEMPLATE SAFETY v2 — interpolación segura + validación
// ═══════════════════════════════════════════════════════════════════════════════

// Whitelist estricta: SOLO estas variables pueden interpolarse en plantillas
const ALLOWED_TEMPLATE_VARS = new Set(['nombre', 'importe', 'dias', 'numero', 'bizum']);

/**
 * Interpolación SEGURA — solo variables de la whitelist.
 * Cualquier {variable} no reconocida se deja literal (no se reemplaza).
 * NUNCA se inyecta contenido libre/LLM en las plantillas.
 */
function interpolateSafe(template: string, vars: Record<string, string>): string {
...
/**
 * Valida una plantilla antes de guardarla en config.
 * Rechaza HTML, JS, URLs externas, variables desconocidas.
 */
function validateTemplate(template: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  if (/<[a-z][^>]*>/i.test(template)) {
    errors.push('La plantilla contiene HTML. Solo texto plano.');
  }
  if (/javascript:|on\w+\s*=/i.test(template)) {
    errors.push('La plantilla contiene código JavaScript.');
  }
  if (/https?:\/\/(?!diabolus\.es)/i.test(template)) {
    errors.push('La plantilla contiene URLs externas. Solo se permiten enlaces a diabolus.es.');
  }
  if (template.length > 1000) {
    errors.push('La plantilla es demasiado larga (máx 1000 caracteres).');
  }

  const unknownVars = [...template.matchAll(/\{(\w+)\}/g)]
    .map(m => m[1])
    .filter(v => !ALLOWED_TEMPLATE_VARS.has(v));
  if (unknownVars.length > 0) {
    errors.push(`Variables no reconocidas: {${unknownVars.join('}, {')}}. Válidas: {${[...ALLOWED_TEMPLATE_VARS].join('}, {')}}.`);
  }
...
/**
 * Sanitiza template (belt + suspenders) — strip HTML/JS antes de guardar.
 */
...
/**
 * formatEur — formatea importe para DISPLAY humano (con €).
 * Para WhatsApp templates: usar formatEurPlain() (sin €).
 */
...
/**
 * formatEurPlain — solo número formateado, SIN símbolo €.
 * Usado en variables de WhatsApp templates (regla {{3}}).
 */
...
// ── Cazador Core Logic v2 ──────────────────────────────────────────────────

export async function runCazador(salonId?: string): Promise<{ enviados: number; salones: number; rechazados: number }> {
  ...
  if (!configs?.length) return { enviados: 0, salones: 0, rechazados: 0 };
  ...
  for (const config of configs) {
    ...
    for (const lvl of ['level1_msg', 'level2_msg', 'level3_msg'] as const) {
      const tmpl = config[lvl];
      if (tmpl) {
        const validation = validateTemplate(tmpl);
        if (!validation.valid) {
          // Plantilla inválida → skip entero y alertar al dueño
          ...
          if (salon?.telegram_chat_id) {
            await sendTelegram(
              salon.telegram_chat_id,
              `⚠️ <b>Cazador — Plantilla inválida</b>\n\n${lvl}: ${validation.errors.join(', ')}\n\nCorrige la plantilla en Configuración > Cazador.`
            );
          }
          totalRechazados++;
          continue;
    ...
    const { data: invoices } = await supabase
      .from('invoices')
      .select('*, clients(name, email, phone, cazador_paused_until)')
    ...
    for (const invoice of invoices) {
      ...
      if (client?.cazador_paused_until) {
        ...
        if (pauseUntil > today) {
          const isNegociacion = pauseUntil.getFullYear() >= 2999;
          skippedLines.push(
            `⏸️ ${client.name || 'Desconocido'} — ${isNegociacion ? 'En negociación' : `Pausado hasta ${pauseUntil.toLocaleDateString('es-ES')}`}`
          );
          continue;
        }
      }
      ...
      // ── INTERPOLACIÓN SEGURA — solo whitelist ──
      const vars = {
        ...
        bizum: salon.bizum_number || '',
      };

      const msgTemplate = level === 1 ? config.level1_msg : level === 2 ? config.level2_msg : config.level3_msg;
      let mensaje = interpolateSafe(msgTemplate, vars);

      // Append Bizum if configured and not already in template
      ...
      if (channel === 'email' && client?.email) {
        ...
        // WhatsApp: usar plantilla aprobada por Meta cuando esté disponible
        // Por ahora: marcar como pendiente de implementación
        skippedLines.push(`📱 ${client.name} — WhatsApp pendiente de plantillas Meta`);
        totalRechazados++;
        continue;
      }
      ...
      if (sent) {
        ...
        await supabase.from('audit_log').insert({
          ...
          result: { sent: true, message_length: mensaje.length },
          confirmed: true, // Pre-autorizado por config del dueño
          level: 1,
        });
  ...
  return { enviados: totalEnviados, salones: configs.length, rechazados: totalRechazados };
}
...
cazadorRoutes.put('/config', async (c) => {
  ...
  // ── v2: VALIDAR plantillas antes de guardar ──
  const templateErrors: string[] = [];
  for (const field of ['level1_msg', 'level2_msg', 'level3_msg'] as const) {
    if (body[field]) {
      const validation = validateTemplate(body[field]);
      if (!validation.valid) {
        templateErrors.push(`${field}: ${validation.errors.join(', ')}`);
      }
    }
  }
  ...
  if (templateErrors.length > 0) {
    return c.json({
      error: 'Plantillas inválidas',
      details: templateErrors,
      hint: `Variables permitidas: {${[...ALLOWED_TEMPLATE_VARS].join('}, {')}}`
    }, 400);
  }

  // Sanitizar por seguridad extra
  const payload = {
    salon_id,
    enabled: body.enabled ?? true,
    level1_days: Number(body.level1_days) || 1,
    level1_msg: sanitizeTemplate(body.level1_msg || ''),
    level2_days: Number(body.level2_days) || 3,
    level2_msg: sanitizeTemplate(body.level2_msg || ''),
    level3_days: Number(body.level3_days) || 7,
    level3_msg: sanitizeTemplate(body.level3_msg || ''),
    ...
    for (const invoice of (invoices || [])) {
      ...
      if (client?.cazador_paused_until) {
        ...
        if (pauseUntil > today) {
          const isNegociacion = pauseUntil.getFullYear() >= 2999;
          previewLines.push(
            `⏸️ ${client.name || 'Desconocido'} — ${formatEur(invoice.total || 0)} — ${isNegociacion ? 'En negociación' : `Pausado hasta ${pauseUntil.toLocaleDateString('es-ES')}`}`
          );
          continue;
        }
      }
