...
import { INVOICE_STATUS, OVERDUE_QUERY_STATUS, mapStatusToDB } from './invoice-status'
...
function extractWithRegex(userInput: string, intent: string): ExtractedData {
  ...
  // Estado deseado (para cambiar_estado) — mapea español → DB via INVOICE_STATUS
  let estadoDeseado: string | null = null
  if (accion === 'cambiar_estado') {
    // Extraer palabra de estado del input
    let rawEstado = 'pagada' // default más común
    if (/vencid/i.test(userInput)) rawEstado = 'vencida'
    else if (/anuld|cancel/i.test(userInput)) rawEstado = 'anulada'
    else if (/pendiente/i.test(userInput)) rawEstado = 'pendiente'
    else if (/pagad|cobrad/i.test(userInput)) rawEstado = 'pagada'
  ...
  if (!invoice && extracted.cliente) {
    ...
    if (clientes?.length) {
      const { data: facturas } = await supabase
        ...
        .in('status', [INVOICE_STATUS.DRAFT, INVOICE_STATUS.SENT])
...
async function fetchOverdue(salonId: string): Promise<string> {
  try {
    ...
    const { data: invoices } = await getSupabase()
      ...
      .eq('salon_id', salonId)
      .eq('status', OVERDUE_QUERY_STATUS)
      .lt('due_date', now)
